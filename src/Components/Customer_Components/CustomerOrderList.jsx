...
const CustomerOrderList = () => {
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        const mappedOrders = response.data.data.map((order) => {
          ...
          const depositCheckRejectReason = depositCheck 
            ? (depositCheck.rejectReason ?? depositCheck.RejectReason ?? '').toString().trim()
            : '';
  ...
  useEffect(() => {
    ...
    const { openOrderId, openRejectOrderId, highlightOrderId, openOrderDetailId } = location.state || {};
    ...
    if (openOrderId || openRejectOrderId || highlightOrderId || openOrderDetailId) {
  ...
  const getPaymentStatusLabel = (status) => {
    ...
    switch (status) {
      ...
      case 2:
        ...
        return 'Đã tt 1 phần'; // PartiallyPaid
...
const getPaymentStatusCodeByContext = (paymentStatus, orderStatus, depositInfo, orderData = null) => {
  ...
  if ((effectiveDepositAmount === 0 || effectiveDepositAmount === null) && depositPercentNum !== null && depositPercentNum > 0 && totalAmount > 0) {
...
const getPaymentStatusLabelByContext = (paymentStatus, orderStatus, depositInfo, orderData = null, short = false) => {
  ...
  if (statusCode === 0 && orderData) {
    ...
    const hasDepositRequirement =
      ...
      (depositPercentNum !== null && depositPercentNum > 0) || depositAmountVal > 0;
  ...
  return short ? getPaymentStatusLabelShort(statusCode) : getPaymentStatusLabel(statusCode);
    ...
    try {
      ...
      if (response.data && response.data.data) {
        ...
        try {
          ...
          const dcList = dcRes?.data?.data ?? [];
          if (Array.isArray(dcList)) {
            const salesOrderId = data.id ?? data.salesOrderId ?? data.SalesOrderId ?? orderId;
            const related = dcList
              .filter((dc) => (dc.salesOrderId ?? dc.SalesOrderId) === salesOrderId)
              .sort((a, b) => {
                const aTime = new Date(a.requestedAt ?? a.RequestedAt ?? 0).getTime();
                const bTime = new Date(b.requestedAt ?? b.RequestedAt ?? 0).getTime();
                return bTime - aTime;
            ...
            if (related.length > 0) {
              ...
              if (latest.status === 2 || latest.Status === 2) { // 2 = Từ chối
                ...
                try {
                  ...
                  const detail = detailRes?.data?.data ?? null;
                  if (detail) {
                    depositCheckRejectReason = (detail.rejectReason ?? detail.RejectReason ?? '').toString().trim();
                  } else {
                    depositCheckRejectReason = (latest.rejectReason ?? latest.RejectReason ?? '').toString().trim();
                  }
                } catch {
                  depositCheckRejectReason = (latest.rejectReason ?? latest.RejectReason ?? '').toString().trim();
                }
    ...
    <Container maxWidth="xl" sx={{ py: { xs: 2, sm: 3, md: 4 }, px: { xs: 1, sm: 2, md: 3 } }}>
          ...
          <div className="customer-order-list-container">
            ...
            <TableHead>
              ...
              <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                ...
                <TableCell sx={{ width: '20%', py: 1.5, px: 0.01, whiteSpace: 'nowrap', textAlign: 'right' }}>
                ...
                paginatedOrders.map((order, index) => (
                  ...
                  <TableCell>
                    ...
                    {order.orderStatus === 0 || order.orderStatus === 1 || order.orderStatus === 3 ? (
                      ...
                      const statusCode = getPaymentStatusCodeByContext(order.paymentStatus, effectiveStatus, {
                      ...
                      const labelFull = getPaymentStatusLabelByContext(order.paymentStatus, effectiveStatus, {
                      ...
                      const labelShort = getPaymentStatusLabelByContext(order.paymentStatus, effectiveStatus, {
                          ...
                          {orderDetails.depositCheckRejectReason && detailOrderStatus === 2 && statusCode === 0 && (
                            <Tooltip title="Xem lý do từ chối thanh toán" arrow>
                              <IconButton
                                size="small"
                                onClick={() => {
                                  setSelectedRejectReason(orderDetails.depositCheckRejectReason);
                                  setIsPaymentRejectReason(true);
                                  setReasonDialogOpen(true);
                                }}
                                sx={{
      ...
      >
        ...
        <DialogTitle className="customer-order-reject-reason-dialog-title">
          {isPaymentRejectReason ? 'Lý do từ chối thanh toán' : 'Lý do từ chối đơn hàng'}
        </DialogTitle>
