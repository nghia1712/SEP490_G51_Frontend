...
const CustomerOrderList = () => {
  ...
  const [isPaymentRejectReason, setIsPaymentRejectReason] = useState(false); // Phân biệt lý do từ chối thanh toán vs đơn hàng
  ...
  const detailOrderStatus =
    orderDetails?.status ?? orderDetails?.Status ?? null;

  const detailPaymentStatus =
    orderDetails?.paymentStatus ?? orderDetails?.PaymentStatus ?? null;
  ...
  const detailPaymentLabel = useMemo(() => {
    ...
    const depositAmountValue =
      toNumberOrNull(
        orderDetails.depositAmount ?? orderDetails.DepositAmount ?? null
      ) ?? 0;
  ...
  const fetchQuotationInfo = useCallback(
    async (quotationId) => {
      ...
      try {
        ...
        console.warn(
          "Không thể lấy thông tin báo giá cho đơn hàng:",
          quotationId,
          error
        );
  ...
  const buildDepositInfo = useCallback(
    async (orderData, totalAmountHint) => {
      ...
      let depositAmount = toNumberOrNull(
      ...
      let remainingDeposit = toNumberOrNull(
        orderData.remainingDeposit ?? orderData.RemainingDeposit ?? null
      );

      let depositDueDays = toNumberOrNull(
        orderData.depositDueDays ?? orderData.DepositDueDays ?? null
      );

      let depositExpiredDate =
        orderData.depositExpiredDate ?? orderData.DepositExpiredDate ?? null;
      ...
      if (
        ...
        depositPercent = Number(
          ((depositAmount / totalAmount) * 100).toFixed(2)
        );
      }

      remainingDeposit =
        depositAmount !== null ? Math.max(0, depositAmount - paidAmount) : 0;
      ...
      if (
        ...
        try {
          ...
          if (Array.isArray(dcList)) {
            // Tạo map: salesOrderId -> latest rejected deposit check
            dcList.forEach((dc) => {
              ...
              if (orderId && status === 2) {
                ...
                const dcTime = new Date(
                  dc.requestedAt ?? dc.RequestedAt ?? 0
                ).getTime();
                const existingTime = existing
                  ? new Date(
                      existing.requestedAt ?? existing.RequestedAt ?? 0
                    ).getTime()
                  : 0;
                if (!existing || dcTime > existingTime) {
                  depositChecksMap[orderId] = dc;
                }
              }
            });
          }
        } catch (e) {
          console.warn("Không thể lấy thông tin deposit checks:", e);
        }
        ...
        const mappedOrders = response.data.data.map((order) => {
          ...
          // Lấy reject reason từ deposit check nếu có
          const depositCheck = depositChecksMap[id];
          const depositCheckRejectReason = depositCheck
            ? (depositCheck.rejectReason ?? depositCheck.RejectReason ?? "")
                .toString()
                .trim()
            : "";

          // Nếu có deposit check bị reject, không hiển thị "Chờ xác nhận" nữa
          if (depositCheck && depositCheck.status === 2) {
            // 2 = Từ chối
          ...
          if (
            ...
            (paymentStatus !== 0 || paidAmountValue > 0)
          ) {
            manualConfirmPending = false;
          ...
          return {
            ...
            depositCheckRejectReason, // Lý do từ chối thanh toán từ deposit check
          };
        });
  ...
  useEffect(() => {
    ...
    // Mở dialog chi tiết đơn hàng khi click vào notification về deposit check reject
    if (openOrderDetailId) {
      const normalizedId = Number(openOrderDetailId);
      if (!Number.isNaN(normalizedId)) {
        setTimeout(() => {
          handleView(normalizedId);
        }, 500);
      }
    }
  ...
  const sortedOrders = useMemo(() => {
    ...
    return [...orders].sort((a, b) => {
      ...
      if (effectiveSortConfig.key === "quotationCode") {
        ...
        aValue =
          a.orderStatus !== undefined && a.orderStatus !== null
            ? a.orderStatus
        ...
        bValue =
          b.orderStatus !== undefined && b.orderStatus !== null
            ? b.orderStatus
        ...
        aValue =
          a.paymentStatus !== undefined && a.paymentStatus !== null
            ? a.paymentStatus
            : -1;

        bValue =
          b.paymentStatus !== undefined && b.paymentStatus !== null
            ? b.paymentStatus
            : -1;
  ...
  const handleEdit = async (orderId) => {
    ...
    try {
      ...
      if (data.salesQuotationId) {
        try {
          const quotationDetailResponse = await salesQuotationAPI.viewDetails(
            data.salesQuotationId
          );
          quotationInfoData = quotationDetailResponse.data?.data ?? null;
          quotationDetailsList =
            quotationInfoData?.Details ?? quotationInfoData?.details ?? [];
        } catch (quotationErr) {
          console.warn(
            "Không thể lấy chi tiết báo giá để bổ sung dữ liệu đơn hàng",
            quotationErr
          );
        }
      }
      ...
      const mergedRows = processedDetails.map((detail, index) => {
        const detailLotId =
          detail.lotId ?? detail.LotId ?? detail.lotID ?? detail.LotID ?? null;
        // Try to match by LotId first, then fallback to index
        let matchedQuotationDetail = null;
        if (detailLotId !== null) {
          matchedQuotationDetail =
            quotationDetailsList.find(
              (qd) =>
        ...
        const quantityValue = Number.isFinite(parsedQuantity)
          ? parsedQuantity
          : 0;
        ...
        if (!productId) {
          console.error(
            "ProductId not found for product:",
            detail.productName,
            {
              ...
              quotationDetailsListSample: quotationDetailsList[0]
                ? Object.keys(quotationDetailsList[0])
                : null,
            }
          );
        }
  ...
  const editQuantitiesTotals = useMemo(() => {
    const before = editQuantitiesRows.reduce(
      (sum, row) => sum + (row.subtotal || 0),
      0
    );
    const after = editQuantitiesRows.reduce(
      (sum, row) => sum + (row.subtotalAfterTax || 0),
      0
    );
  ...
  const handleSend = async (orderId) => {
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Không thể gửi đơn hàng";
        ...
        try {
          ...
          if (Array.isArray(dcList) && dcList.length > 0) {
            const related = dcList
              ...
              .filter(
                (dc) =>
                  dc.salesOrderId === salesOrderId ||
                  dc.SalesOrderId === salesOrderId
              )

              .sort((a, b) => {
                const aTime = new Date(
                  a.requestedAt ?? a.RequestedAt ?? 0
                ).getTime();

                const bTime = new Date(
                  b.requestedAt ?? b.RequestedAt ?? 0
                ).getTime();
            ...
            if (related.length > 0) {
              ...
              if (latest.status === 2 || latest.Status === 2) {
                ...
                try {
                  const detailRes =
                    await paymentAPI.getManualDepositCheckDetail(latestId);
                  ...
                  if (detail) {
                    ...
                    setLatestDepositCheck({
                      ...
                      rejectReason:
                        latest.rejectReason ?? latest.RejectReason ?? null,
                    });
                  }
                } catch {
                  setLatestDepositCheck({
          ...
          console.error(
            "Không thể lấy thông tin yêu cầu xác nhận thanh toán:",
            e
          );
        }
  ...
  const handleConfirmPayment = async () => {
    ...
    if (!paymentOrderDetails?.id) {
      setSnackbarMessage(
        "Không tìm thấy thông tin đơn hàng để xác nhận thanh toán."
      );
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((order) =>
          order.id === targetId
            ? { ...order, manualConfirmPending: true }
            : order
        )
      );
  ...
  const handleComplete = async (orderId) => {
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Không thể hoàn thành đơn hàng";
      ...
      if (response.data && response.data.data) {
        ...
        const mergedDetails = processedDetails.map((detail, index) => {
          ...
          if (matchedQuotationDetail) {
            ...
            if (
              quotationSalesPrice !== null &&
              quotationSalesPrice !== undefined
            ) {
            ...
            if (
              ...
              unitPriceAfterTax === undefined
            ) {
              unitPriceAfterTax =
                detail.unitPriceAfterTax ?? unitPriceBeforeTax;

              subtotalAfterTax =
                subtotalAfterTax ??
                quantityValue *
                  (unitPriceAfterTax !== null && unitPriceAfterTax !== undefined
                    ? unitPriceAfterTax
                    : 0);
            }
        ...
        // Lấy reject reason từ deposit check nếu có
        let depositCheckRejectReason = "";
        try {
          const dcRes = await paymentAPI.getManualDepositChecks();
          const dcList = dcRes?.data?.data ?? [];
          if (Array.isArray(dcList)) {
            const salesOrderId =
              data.id ?? data.salesOrderId ?? data.SalesOrderId ?? orderId;
            const related = dcList
              .filter(
                (dc) => (dc.salesOrderId ?? dc.SalesOrderId) === salesOrderId
              )
              .sort((a, b) => {
                const aTime = new Date(
                  a.requestedAt ?? a.RequestedAt ?? 0
            ...
            if (related.length > 0) {
              ...
              if (latest.status === 2 || latest.Status === 2) {
                ...
                try {
                  const detailRes =
                    await paymentAPI.getManualDepositCheckDetail(
                      latest.id ?? latest.Id
                    );
                  const detail = detailRes?.data?.data ?? null;
                  if (detail) {
                    depositCheckRejectReason = (
                      detail.rejectReason ??
                      detail.RejectReason ??
                      ""
                    )
  ...
  const renderActions = (order) => {
    ...
    const totalAmount =
      toNumberOrNull(
        order.totalAmount ?? order.TotalAmount ?? order.grandTotal ?? 0
      ) ?? 0;
    const paidAmount =
      toNumberOrNull(order.paidAmount ?? order.PaidAmount ?? 0) ?? 0;
    ...
    const paymentStatusCode = getPaymentStatusCodeByContext(
      order.paymentStatus,
      effectiveStatus,
      {
        percent: order.depositPercent,
    ...
    return (
      ...
      >
        ...
        {order.depositCheckRejectReason &&
          order.orderStatus === 2 &&
          paymentStatusCode === 0 && (
            <Tooltip
              title="Xem lý do từ chối thanh toán"
              placement="bottom"
              arrow
            >
              <IconButton
                size="medium"
                onClick={() => {
                  setSelectedRejectReason(order.depositCheckRejectReason);
                  setIsPaymentRejectReason(true);
                  setReasonDialogOpen(true);
                }}
                sx={{
                  color: "#f57c00",
  ...
  const handleViewRejectReason = useCallback((order) => {
    ...
    setIsPaymentRejectReason(false); // Lý do từ chối đơn hàng
  ...
  const handleCloseRejectReason = () => {
    ...
    setIsPaymentRejectReason(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!loading && (
          <div className="customer-order-list-container">
            ...
            >
              <Table
                className="customer-order-list-table"
              ...
              >
                <TableHead>
                  <TableRow sx={{ backgroundColor: "#f5f5f5" }}>
                    ...
                    <TableCell
                      sx={{ width: "20%", textAlign: "right", py: 1.5, px: 2 }}
                    >
                      <Box
                        sx={{
                          display: "flex",
                    ...
                    paginatedOrders.map((order, index) => (
                      ...
                      >
                        ...
                        <TableCell sx={{ fontWeight: 500 }}>
                          {order.quotationCode || order.quotationId || "-"}
                        </TableCell>
                        ...
                        <TableCell
                          sx={{ width: "15%", textAlign: "right", pr: 4 }}
                        >
        ...
        <DialogContent className="customer-order-detail-dialog-content">
          {detailLoading ? (
            ...
            <Box>
              ...
              <Box
                className="customer-order-detail-info-layout"
                        ...
                        orderDetails.details.map((detail, index) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell sx={{ textAlign: "right" }}>
                                {formatCurrency(unitPriceAfterTax)}
                              </TableCell>
      ...
      >
        <DialogTitle
          className="customer-order-payment-dialog-title"
          sx={{ textAlign: "left", fontWeight: 600, fontSize: "1.25rem" }}
        >
        ...
        <DialogContent className="customer-order-payment-dialog-content">
          {paymentLoading ? (
            ...
            <Box>
              ...
              <Box
                className="customer-order-payment-info-layout"
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    {paymentOrderDetails.paymentStatus !== undefined &&
                    paymentOrderDetails.paymentStatus !== null ? (
                      <Chip
                        label={getPaymentStatusLabelByContext(
                          paymentOrderDetails.paymentStatus,

                          paymentOrderDetails.status ??
                            paymentOrderDetails.orderStatus ??
                            null,
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body1">
                      {paymentOrderDetails.expiredDate
                        ? formatDate(paymentOrderDetails.expiredDate)
                        : "-"}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body1">
                      {paymentOrderDetails.depositPercent !== undefined &&
                      paymentOrderDetails.depositPercent !== null
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      {formatCurrency(
                        getDepositAmountValue(paymentOrderDetails)
                      )}
                    </Typography>
                  </Box>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Alert severity="warning">
                        ...
                        <Typography variant="body2" sx={{ mt: 1 }}>
                          Bạn có thể kiểm tra lại thông tin chuyển khoản và gửi
                          ...
                          <>
                            <Box sx={{ textAlign: "center" }}>
                              <Typography variant="h6" sx={{ fontWeight: 600 }}>
                                {formatCurrency(
                                  vnPayInitData?.amount ??
                                    paymentOrderDetails.remainingDeposit
                                )}
                              </Typography>
                            </Box>
                      ...
                      >
                        <Alert severity="info">
                          ...
                          <Typography variant="body2">
                            ...
                            đến tài khoản <strong>4619300024210402</strong> -
                            chủ sở hữu <strong>NGUYEN QUANG TRUNG</strong> -
                            Ngân hàng <strong>Timo</strong>.
                          </Typography>

                          <Typography variant="body2" sx={{ mt: 1 }}>
                            Sau khi thanh toán thành công, vui lòng chờ nhân
                            viên để xác nhận thanh toán hoặc liên hệ với nhân
                            viên qua số điện thoại <strong>0398233047</strong>.
                          </Typography>
                        </Alert>
                      </Box>
                    )}
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle className="customer-order-reject-reason-dialog-title">
          {isPaymentRejectReason
            ? "Lý do từ chối thanh toán"
            : "Lý do từ chối đơn hàng"}
        </DialogTitle>

        <DialogContent
          className="customer-order-reject-reason-dialog-content"
          dividers
        >
      ...
      >
            ...
            <Box>
              ...
              {Object.keys(editQuantitiesErrors).length > 0 && (
                <Alert severity="error" sx={{ mb: 2 }}>
                  ...
                  {Object.entries(editQuantitiesErrors).map(
                    ([key, message]) => {
                      const rowId = key.replace("row_", "");
                      const row = editQuantitiesRows.find(
              ...
              <Box sx={{ mb: 2, textAlign: "right" }}>
                ...
                >
                  ...
                  <Box component="span">
                    {formatCurrency(editQuantitiesTotals.before)}
                  </Box>
                </Typography>
                <Typography
                  variant="body1"
                ...
                >
                  <span>Thuế:</span>
                  <Box component="span">
                    {formatCurrency(editQuantitiesTotals.tax)}
                  </Box>
                </Typography>
                <Box
                  sx={{
        ...
        <DialogActions>
          <Button
            onClick={handleCloseEditQuantitiesDialog}
            disabled={editQuantitiesLoading}
          >
            Hủy
          </Button>
          <Button
