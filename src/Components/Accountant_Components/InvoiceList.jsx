...
const InvoiceList = () => {
    ...
    try {
      ...
      if (Array.isArray(invoiceList)) {
        const mappedInvoices = invoiceList.map((invoice) => ({
          ...
          orderCode: invoice.salesOrderCode || invoice.SalesOrderCode || `SO-${invoice.salesOrderId || invoice.SalesOrderId}`,
          status: invoice.status !== undefined ? invoice.status : (invoice.Status !== undefined ? invoice.Status : 0),
          createdAt: invoice.createdAt || invoice.CreatedAt || invoice.createAt || invoice.CreateAt,
  ...
  const handleSend = async (invoiceId) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Không thể gửi hóa đơn';
      setSnackbarMessage(errorMessage);
      setSnackbarOpen(true);
    }
  };
