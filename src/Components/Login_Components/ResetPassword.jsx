  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        setStatusMessage(
          "Đặt lại mật khẩu thất bại do bạn xác nhận email cũ.\nHãy đổi mật khẩu lại để nhận email xác nhận mới."
        );
      } else if (error.response?.status === 404) {
        setStatusMessage("Không tìm thấy người dùng");
      } else if (error.response?.status === 500) {
        setStatusMessage(
          "Đặt lại mật khẩu thất bại do token đã được sử dụng hoặc không hợp lệ"
        );
      } else {
        setStatusMessage(
                ...
                <AnimatePresence>
                  {statusMessage && (
                    <motion.div
                      ...
                      initial="initial"
                      animate={["animate", isError ? "shake" : "animate"]}
                      exit="exit"
                    >
                      <Alert
